...
void DecisionTree::build(std::vector<Example> train_data,
    ...
    std::cout << "Recursion in tree" << std::endl;
...
void DecisionTree::print(DecisionTreeNode *p, std::string parent_path) {
    if (p -> getType() == "discrete") {
        ...
        for (int i = 0; i < child_pointers.size(); i++) {
            print(child_pointers[i], parent_path + " -> " + child_pointers[i]->getAttrName());
        }
